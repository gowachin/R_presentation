...
.pull-left[
Background personnel : Apprentissage de R pour l'**écologie**, traitements de données.

Bonnes pratiques viennent de la pratique, des échanges et de la communauté.
]

.pull-right[
```{r, out.width="100%"}
knitr::include_graphics("CSU_RepRo_files/figure-html/papaillon.jpg")
```
]
...
Seule exception : les données de type cache : permet de faire tourner les scripts plus facilement. Package `{drake}` pour refaire tourner que les parties modifiées ou dépendantes d'un script.
